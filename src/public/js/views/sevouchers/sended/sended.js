...
$(document).ready(function () {
    const sevoucher_table = $("#sevouchers").DataTable({
        ...
        processing: true,
        ajax: {
            url: "/datatablesQuery/sevouchers/sended",
            data: (d) => {
                return $.extend({}, d, {
        ...
        columnDefs: [
            ...
            },
            {
                ...
                render: (data, types, row) => {
                    ...
                    return `<i class="text-${color}">${types_array[0]}: ${data}</i><br><b>${row["sevoucher_no"]}</b>`;
                },
            },
            {
                targets: 3,
                ...
                try {
                    ...
                    fetch(blobUrl)
                        ...
                        .then(async (html) => {
                            ...
                            await worker.set({
                                margin: 0.2,
                                filename: `${data.sevoucher_no} - ${data.receiver_name}.pdf`,
                                html2canvas: {
                                    width: 800,
                                    logging: false,
                                },
    ...
    $("#sevouchers tbody").on("click", "#check-status", function () {
        let data = sevoucher_table.row($(this).parents("tr")).data();
        $.ajax({
            type: "GET",
            url: `/sevouchers/sended/${data.id}/check/status`,
            success: function (response) {
                console.log("response :>> ", response);
                Swal.fire({
                    title: response.summary,
                    text: `${data.sevoucher_no} numaralı SM makbuzunun durumu`,
                    icon: "info",
                });
            },
    ...
    $("#sevouchers tbody").on("click", "#mark-not-sended", function () {
        let data = sevoucher_table.row($(this).parents("tr")).data();
        Swal.fire({
            title: `${data.erp_no} numaralı SM makbuzunu gönderilmedi olarak işaretlemek istediğinize emin misiniz?`,
            icon: "question",
            ...
            preConfirm: () => {
                return $.ajax({
                    type: "DELETE",
                    url: `/sevouchers/sended/${data.id}/mark/send`,
                    success: function (response) {
                        return true;
                    },
                });
    ...
    $("#sevouchers tbody").on("click", "#mark-resolved", function () {
        let data = sevoucher_table.row($(this).parents("tr")).data();
        Swal.fire({
            title: `${data.erp_no} numaralı SM makbuzunu çözüldü olarak işaretlemek istediğinize emin misiniz?`,
            icon: "question",
            showCancelButton: true,
            confirmButtonText: `Evet`,
            cancelButtonText: `Hayır`,
            preConfirm: () => {
                return $.ajax({
                    type: "PUT",
                    url: `/sevouchers/sended/${data.id}/mark/resolve`,
                    success: function (response) {
                        return true;
                    },
                });
    ...
    $("#sevouchers tbody").on("click", "#status-label", function () {
        let data = sevoucher_table.row($(this).parents("tr")).data();
    ...
    $("#coll-pdf-one-page-download").click((e) => {
        ...
        Swal.fire({
            icon: "question",
            title: "Emin misiniz?",
            text: `${clicked.length} adet SM makbuzunu tek sayfa PDF olarak indirmek istediğinize emin misiniz?`,
            showConfirmButton: true,
            ...
            if (result.isConfirmed) {
                ...
                clicked.each(async (id, index) => {
                    promises.push(async () => {
                        let data = sevoucher_table.data().filter((e) => e.id == id)[0];
                        let xml = await $.ajax({
                            type: "GET",
                            url: `/sevouchers/waiting/${data.id}/xml`,
                        });
                ...
                await Promise.all(promises.map((p) => p())).then(() => {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            const fileName1 = `Tek Sayfa PDF Gönderilmiş SM Makbuzları.pdf`;
    ...
    $("#coll-pdf-download").click((e) => {
        ...
        Swal.fire({
            icon: "question",
            title: "Emin misiniz?",
            text: `${clicked.length} adet SM makbuzunu toplu PDF olarak indirmek istediğinize emin misiniz?`,
            showConfirmButton: true,
            confirmButtonText: "Evet, indir",
            showCancelButton: true,
            cancelButtonText: "Hayır, indirme",
                ...
                clicked.each(async (id, index) => {
                    promises.push(async () => {
                        ...
                        try {
                            ...
                            zip.file(
                                `${data.sevoucher_no} - ${data.receiver_name}.pdf`,
                                pdf_blob
                            );
                            counter += 0.4;
                            $("#progress-sending-selected")
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            zip.generateAsync({
                                ...
                                saveAs(
                                    zipBlob,
                                    "Toplu PDF Gönderilmiş SM Makbuzları.zip",
                                    "application/zip"
                                );
                            });
                        }
