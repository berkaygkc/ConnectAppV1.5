$(document).ready(function () {
    $("#esevoucher-serie-name").inputmask("(A|9)(A|9)[A|9]");

    const esevoucher_table = $("#esevoucher-series-table").DataTable({
        language: {
        ...
        ordering: false,
        ajax: {
            url: "/datatablesQuery/definitions/series/5",
        },
        columns: [
            { data: "id" },
            { data: "name" },
    ...
    $("#esevoucher-save-serie").click((e) => {
        const serieName = $("#esevoucher-serie-name").val();
        let errors = "";
        if (!serieName) {
            errors += "- Seri kodu belirlemek zorunludur! <br>";
        }
        if (!$("#esevoucher-serie-name").inputmask("isComplete")) {
        ...
        formData.append("is_default", $("#esevoucher-isdefault").is(":checked"));
        fetch("/definitions/series/5", {
            method: "POST",
            body: formData,
        })
            .then((res) => res.text())
            .then((result) => {
                ...
                if (response.status) {
                    Swal.fire({
                        ...
                        $("#esevoucher-serie-name").val("");
                        esevoucher_table.draw();
                        $("#esevoucher-serie-modal").modal("hide");
                    });
                } else {
                    Swal.fire(
                        "Hata!!",
    ...
    $("#esevoucher-series-table tbody").on("click", "#make-default", function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "PUT",
                    url: `/definitions/series/5/${data.id}/default`,
    ...
    $("#esevoucher-series-table tbody").on("click", "#make-aktif", function () {
        let data = esevoucher_table.row($(this).parents("tr")).data();
        Swal.fire({
            title:
                data.name +
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "PUT",
                    url: `/definitions/series/5/${data.id}/status`,
                    success: function (response) {
                        esevoucher_table.draw();
                        toastr["success"](
    ...
    $("#esevoucher-series-table tbody").on("click", "#make-pasif", function () {
        let data = esevoucher_table.row($(this).parents("tr")).data();
        Swal.fire({
            title:
                data.name +
