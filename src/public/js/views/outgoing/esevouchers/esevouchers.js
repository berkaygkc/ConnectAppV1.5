...
$(document).ready(function () {
    ...
    const sevoucher_table = $("#sevouchers").DataTable({
        ...
        processing: true,
        ajax: {
            url: "/datatablesQuery/documents/sevouchers",
            data: (d) => {
                return $.extend({}, d, {
                    query: {
                        document: "esevoucher",
        ...
        columnDefs: [
            ...
            },
            {
                ...
                render: (data, type, row) => {
                    ...
                    <div class="btn-group" role="group" aria-label="Basic example">
                        <button id="preview-sevoucher" type="button" class="btn btn-icon btn-label-info waves-effect waves-light preview-button">
            ...
            },
            {
                ...
                render: (data, type, row) => {
                    ...
                    return `<i class="fa-solid fa-hashtag fa-sm"></i> <b>${data}</b><br><span class="badge bg-${type_class}">${
                        row["sev_type"].length > 8
                            ? row["sev_type"].substring(0, 8) + "."
                            : row["sev_type"]
                    }</span>`;
                },
            },
            ...
            },
            {
                ...
                width: "1px",
                render: (data, type, row) => {
                    return `<b>e-SMM: </b>${data}<br><b>Zarf: </b>${row["sev_env_date"]}`;
                },
            },
            {
                targets: 10,
                width: "1px",
                render: (data, type, row) => {
                    return `<b>Toplam: </b>${data} ${row["sev_currency"]}`;
                },
            },
            {
                targets: 13,
    ...
    const searchTable = () => {
        ...
        let query = sevoucher_table.ajax.params().query;
    ...
    $("#sevouchers tbody").on("click", "#preview-sevoucher", function (e) {
        ...
        $("#sevoucher-preview-uuid").val(data.id);
        $("#sevoucher-preview-sev-no").val(data.sev_no);
        $("#sevoucher-preview-integrator").val(data.integrator);
        let blob, detail;
        $.when(
            $.ajax({
                type: "GET",
                url: `/documents/sevouchers/export?uuid=${data.id}&type=html&document=esevoucher&coding=decoded&integrator=${data.integrator}`,
                success: function (response) {
                    blob = new Blob([response], {
                        type: "text/html",
                    });
                    $("#sevoucherFrame").attr("src", URL.createObjectURL(blob));
                    $("#sevoucherFrame").slideDown(500);
                    setTimeout(() => {
                        let iframe = document.querySelector("#sevoucherFrame");
                        iframe.style.width =
                            String(
                                Number(
            ...
            $.ajax({
                type: "GET",
                url: `/documents/sevouchers/detail?uuid=${data.id}&dir=out&document=esevoucher`,
                success: function (response) {
            ...
            //     ? null //$("#cancel-sevoucher-in-preview").slideDown(100)
            //     : null;
    ...
    $("#preview-sevoucher-modal").on("hide.bs.modal", function () {
        $("#print-label").empty();
        $("#system-status-label").empty();
        // $("#gib-status-label").empty();
        $("#cancel-sevoucher-in-preview").unbind();
        $("#cancel-sevoucher-in-preview").remove();
        $("#sevoucher-preview-uuid").val("");
        $("#sevoucher-preview-sev-no").val("");
        $("#sevoucher-preview-integrator").val("");
    });
    ...
    $("#preview-body").click((e) => {
        if (
            ...
            $("#preview-sevoucher-modal").modal("hide");
        }
    });

    $("#print-sevoucher-in-preview").click((e) => {
        let uuid = $("#sevoucher-preview-uuid").val();
    ...
    $("#download-sevoucher-in-preview").click((e) => {
        let uuid = $("#sevoucher-preview-uuid").val();
        let sevno = $("#sevoucher-preview-sev-no").val();
        let integrator = $("#sevoucher-preview-integrator").val();
        $.ajax({
            type: "GET",
            url: `/documents/sevouchers/export?uuid=${uuid}&type=pdf&document=esevoucher&integrator=${integrator}`,
            success: function (response) {
                const linkSource = `data:application/pdf;base64,${response.content}`;
                const downloadLink = document.createElement("a");
                const fileName = `${sevno}.pdf`;
    ...
    $("#sevouchers tbody").on("click", "#mark-read", function (e) {
        let data = sevoucher_table.row($(this).parents("tr")).data();
        markInvoice([data.id], "out", "read", true);
    });
    $("#sevouchers tbody").on("click", "#mark-unread", function (e) {
        let data = sevoucher_table.row($(this).parents("tr")).data();
        markInvoice([data.id], "out", "read", false);
    });
    $("#sevouchers tbody").on("click", "#mark-print", function (e) {
        let data = sevoucher_table.row($(this).parents("tr")).data();
        markInvoice([data.id], "out", "print", true);
    });
    $("#sevouchers tbody").on("click", "#mark-unprint", function (e) {
        let data = sevoucher_table.row($(this).parents("tr")).data();
        markInvoice([data.id], "out", "print", false);
    });
    $("#sevouchers tbody").on("click", "#pdf-download", function (e) {
        let data = sevoucher_table.row($(this).parents("tr")).data();
        $.ajax({
            type: "GET",
            url: `/documents/sevouchers/export?uuid=${data.id}&type=pdf&document=esevoucher&integrator=${data.integrator}`,
            success: function (response) {
                const linkSource = `data:application/pdf;base64,${response.content}`;
                const downloadLink = document.createElement("a");
                const fileName = `${data.sev_no}-${data.receiver_name}.pdf`;
                downloadLink.href = linkSource;
                downloadLink.download = fileName;
                downloadLink.click();
            },
    ...
    $("#sevouchers tbody").on("click", "#xml-download", function (e) {
        ...
        $.ajax({
            type: "GET",
            url: `/documents/sevouchers/export?uuid=${data.id}&type=xml&document=esevoucher&integrator=${data.integrator}`,
            success: function (response) {
                const linkSource = `data:application/xml;base64,${response.content}`;
                const downloadLink = document.createElement("a");
                const fileName = `${data.sev_no}.xml`;
                downloadLink.href = linkSource;
                downloadLink.download = fileName;
                downloadLink.click();
            },
    ...
    $("#sevouchers tbody").on("click", "#send-mail", function (e) {
        Swal.fire({
            icon: "question",
            title: "Emin misiniz?",
            text: "e-SMM'yi mail olarak göndermek istediğinize emin misiniz?",
                ...
                $.ajax({
                    ...
                    success: (response) => {
                        ...
                        Swal.fire({
                            icon: "success",
                            title: "Başarılı",
                            text: "e-SMM mail gönderme işlemi sıraya alındı!",
                    ...
                    error: (err) => {
                        ...
                        Swal.fire({
                            icon: "error",
                            title: "Hata",
                            html: `e-SMM mail gönderme işlemi sıraya alınırken bir hata oluştu! 
    ...
    $("#coll-pdf-download").click((e) => {
        let clicked = sevoucher_table.column(0).checkboxes.selected();
        if (clicked.length < 2) {
            toastr["error"](`En az 2 adet e-SMM seçmelisiniz!`, "Uyarı", {
                closeButton: true,
                tapToDismiss: false,
                progressBar: true,
            });
        ...
        Swal.fire({
            icon: "question",
            title: "Emin misiniz?",
            text: `${clicked.length} adet e-SM Makbuzu toplu PDF olarak indirmek istediğinize emin misiniz?`,
            showConfirmButton: true,
            ...
            if (result.isConfirmed) {
                ...
                Swal.fire({
                    ...
                    html: `
                            <h4>e-SM Makbuzlar hazırlanıyor...</h4>
                ...
                clicked.each((id, index) => {
                    ...
                    requests.push({
                        type: "GET",
                        url: `/documents/sevouchers/export?uuid=${id}&type=pdf&document=esevoucher&integrator=${data.integrator}`,
                ...
                ajaxThrottle({
                    ...
                    onAllSettled: () => {
                        Swal.fire({
                            ...
                            html: `
                                    <h4>e-SM Makbuzlar Hazırlandı!</h4>
                                    <blockquote class="blockquote mt-3">
    ...
    $("#coll-xml-download").click((e) => {
        ...
        Swal.fire({
            icon: "question",
            title: "Emin misiniz?",
            text: `${clicked.length} adet e-SM Makbuzu toplu XML olarak indirmek istediğinize emin misiniz?`,
            showConfirmButton: true,
            confirmButtonText: "Evet, indir",
            showCancelButton: true,
            cancelButtonText: "Hayır, indirme",
            ...
            if (result.isConfirmed) {
                ...
                clicked.each((id, index) => {
                    ...
                    requests.push({
                        type: "GET",
                        url: `/documents/sevouchers/export?uuid=${id}&type=xml&document=esevoucher&integrator=${data.integrator}`,
                        success: async (result) => {
    ...
    $("#coll-pdf-one-page-download").click((e) => {
        ...
        Swal.fire({
            icon: "question",
            title: "Emin misiniz?",
            text: `${clicked.length} adet e-SM Makbuzu tek sayfa PDF olarak indirmek istediğinize emin misiniz?`,
            showConfirmButton: true,
            confirmButtonText: "Evet, indir",
            showCancelButton: true,
            cancelButtonText: "Hayır, indirme",
                ...
                ajaxThrottle({
                    ...
                    onAllSettled: () => {
                        Swal.fire({
                            ...
                            const fileName1 = `Tek Sayfa PDF Giden e-SMM.pdf`;
    ...
    $("#coll-sevoucher-xlsx-download").click((e) => {
        let clicked = sevoucher_table.column(0).checkboxes.selected();
        if (clicked.length < 1) {
            toastr["error"](`En az 1 adet e-SMM seçmelisiniz!`, "Uyarı", {
                closeButton: true,
                tapToDismiss: false,
                progressBar: true,
            });
        ...
        Swal.fire({
            icon: "question",
            title: "Emin misiniz?",
            text: `${clicked.length} adet e-SM Makbuzu Excel olarak indirmek istediğinize emin misiniz?`,
            showConfirmButton: true,
            confirmButtonText: "Evet, indir",
            showCancelButton: true,
            cancelButtonText: "Hayır, indirme",
            ...
            if (result.isConfirmed) {
                ...
                clicked.each((id) => {
                    requests.push({
                        type: "GET",
                        url: `/documents/sevouchers/detail?uuid=${id}&dir=out&document=esevoucher`,
                        success: async (result) => {
                            excelData.push(
                                sevoucherDetailToJson(result.document)
                            );
                            successed++;
                            $("#success-counter").text(
                                Number($("#success-counter").text()) + 1
                ...
                ajaxThrottle({
                    ...
                    onAllSettled: () => {
                        Swal.fire({
                            ...
                            saveAs(fileData, "Giden e-SMM" + fileExtension);
                        });
                    },
                    onAnyFailed: () => {},
                    onCancelled: () => {
    ...
    const buttonsCreator = (can_cancelled) => {
        if (can_cancelled != 2009) {
            $("#detail-buttons").append(
                `<button id="cancel-sevoucher-in-preview" class="btn btn-label-pinterest d-grid w-100 mb-2 waves-effect" hidden>
            ...
            $("#cancel-sevoucher-in-preview").click((e) => {
                let sevno = $("#sevoucher-preview-sev-no").val();
                Swal.fire({
                    icon: "question",
                    title: "Emin misiniz?",
                    text: `${sevno} numaralı e-SM Makbuzu iptal etmek istediğinize emin misiniz?`,
                    showConfirmButton: true,
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "/documents/sevouchers/cancel?document=esevoucher",
                            headers: {
                                "Content-type": "application/json",
                            },
                            ...
                            error: (err) => {
                                console.log(err);
                                toastr["error"](
                                    `e-SMM iptal edilirken hata oluştu! ${err.responseJSON?.data[0]?.message}`,
                                    "Hata",
                                    {
                                        closeButton: true,
                                        tapToDismiss: false,
    ...
    const markInvoice = (uuids, direction, mark, value) => {
        ...
        $.ajax({
            type: "POST",
            url: "/documents/sevouchers/mark?document=esevoucher",
            headers: {
                "Content-type": "application/json",
            },
            data: JSON.stringify(body),
    ...
    const sevoucherDetailToJson = (data) => {
        let taxObject = {
            "KDV0 Matrah": 0,
            "KDV0 Tutar": 0,
            "KDV1 Matrah": 0,
        ...
        for (tax of data.tax_totals[0].subtotals) {
            if (tax.code == "0015") {
                if (tax.percent == 0) {
        ...
        return {
            ...
            "Vergiler Toplamı": data.tax_totals[0].amount,
